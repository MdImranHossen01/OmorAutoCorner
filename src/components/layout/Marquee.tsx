  ...
  useEffect(() => {
    ...
    async function fetchSettings() {
      try {
        ...
        setMarqueeText(data.marqueeText || 'Welcome to Omor Auto Corner! Free shipping on orders over ৳500.');
      } catch (error: any) {
        if (error.name !== 'AbortError') {
          console.error('Error fetching marquee text:', error);
          setMarqueeText('Welcome to Omor Auto Corner! Free shipping on orders over ৳500.');
        }
      }
    }
    fetchSettings();
