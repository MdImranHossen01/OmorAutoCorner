...
export const sendResetEmail = async (email: string, token: string) => {
  ...
  const mailOptions = {
    from: `"Omor Auto Corner" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: 'Password Reset Request - Omor Auto Corner',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 10px;">
        <h2 style="color: #0d9488; text-align: center;">Omor Auto Corner</h2>
        <p>Hello,</p>
        ...
        <p style="font-size: 12px; color: #888;">Omor Auto Corner - Your Trusted Online Store</p>
      </div>
    `,
  };
